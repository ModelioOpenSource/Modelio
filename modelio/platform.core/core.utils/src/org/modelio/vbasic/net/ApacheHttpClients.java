 ...
 * <li> Setup proxy authentication from system preferences set by
 * Eclipse preferences dialog with {@link #configProxyCredentials(Properties, String, CredentialsProvider)}.
 * </ul>
 * The HTTP clients are configured to use {@link SslManager} to validate server certificates
 * that may ask the user for confirmation in case of untrusted certificate.
 * 
 * @author cma
...
public class ApacheHttpClients {
     ...
     * <p>
     * There are 3 properties you can set to specify the proxy that will be used by the http protocol handler:<ul>
     ...
     * </ul>
     * <p>
     * The Eclipse preference page allows modifying all of these.
     * <code>proxyUser</code> and <code>proxyPassword</code> are properties not used by the JDK but set by Eclipse preference page.
     ...
     * from user settings and Eclipse preferences stored in System properties..
     * @param credsProvider the credential provider to fill.
    ...
    public static class HostNameVerifier implements X509HostnameVerifier {
        @objid ("5a220cdc-5a1c-453f-ad74-e5fa14cd8a84")
        private final BrowserCompatHostnameVerifier delegate = new BrowserCompatHostnameVerifier();
