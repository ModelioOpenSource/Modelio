...
public class GProject {
     ...
     * <code>done()</code> on the given monitor. Accepts <code>null</code>, indicating that no progress should be
     * reported and that the operation cannot be cancelled.
     * @throws FragmentConflictException if a fragment with same identifier or URI is already registered
     */
     ...
     * @throws IllegalStateException if the project is already opened.
     */
     ...
     * @param aProgress a progress monitor
     * @throws IOException in case of I/O error preventing the project from being instantiated
     */
     ...
     * @throws IOException if the new version is not supported.
     */
    ...
    protected void validateModelioVersion() throws IOException {
        if (this.expectedModelioVersion != null) {
            if (this.expectedModelioVersion.withoutBuild().isNewerThan(ModelioVersion.MAJOR_MINOR)) {
                throw new IOException(CoreProject.I18N.getMessage(
                        "GProject.modelioTooOld",
                        getName(),
                        this.expectedModelioVersion.toString("V.R"),
                        ModelioVersion.MAJOR_MINOR.toString("V.R")));
            }
        }
        
        // If validated writes the new version
        this.expectedModelioVersion = ModelioVersion.MAJOR_MINOR;
     ...
     * @throws FileSystemException on file system error.
     ...
     * @throws IOException in case of fatal I/O failure.
     */
