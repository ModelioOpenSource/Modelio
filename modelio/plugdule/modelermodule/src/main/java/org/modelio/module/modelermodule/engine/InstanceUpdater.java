...
public class InstanceUpdater {
    @objid ("d030175c-3b51-41fb-9e34-90c46b6ad083")
    private Shell obj = null;

    /**
     * Default constructor.
     */
    @objid ("7086ee1d-bd6c-4c32-b717-b17b5f82581e")
    ...
    public boolean createAttribute(final IModelingSession session, final AttributeLink current) throws ModelerModuleException {
        ...
        if (att != null) {
            ...
            if (inst.getBase() instanceof Classifier) {
                ...
                String attName = InputDialog.showInputDialog(this.obj,
        ...
        for (BindableInstance inst : new ArrayList<>(current.getInternalStructure())) {
            if (!(inst instanceof Port)) {
                ...
                if (inst.getBase() != null) {
                    ...
                    ConfirmDialog.Values res = ConfirmDialog.showConfirmDialog(this.obj,
    ...
    public boolean createOperation(final IModelingSession session, final Message current) throws ModelerModuleException {
        ...
        if (op != null) {
            ...
            Lifeline lifeline = current.getReceiveEvent().getCovered().get(0);
            ...
            if (occurence == null) {
                ...
                // If no instance has been created, rollback & quit
                if (occurence == null) {
                    return false;
                }
            }
            ...
            if (current.getInvoked() == null) {
                String opName = InputDialog.showInputDialog(this.obj,
                        I18nMessageService.getString("module.gui.process.operationName"),
                        I18nMessageService.getString("module.gui.process.chooseName"),
                        current.getName());
                if (opName != null && !opName.equals("")) {
                    for (Operation operation : base.getOwnedOperation()) {
    ...
    public boolean createOperation(final IModelingSession session, final Transition current) throws ModelerModuleException {
        ...
        if (op != null) {
            ...
            if (ret instanceof Classifier) {
                ...
                String className = InputDialog.showInputDialog(this.obj,
            ...
            if (o != null) {
                String opName = InputDialog.showInputDialog(this.obj,
                        ...
                        current.getName());
                if (opName != null && !opName.equals("")) {
                    for (Operation tmp : o.getOwnedOperation()) {
    ...
    public boolean createInstanceAndClassifier(final IModelingSession session, final Lifeline current) throws ModelerModuleException {
        ...
        String instName = InputDialog.showInputDialog(this.obj,
    ...
    private Port createPortFromPort(final IModelingSession session, final Classifier current, final Port sourcePort) {
        Port ret = session.getModel().createPort();
        sourcePort.setRepresentedFeature(ret);
        current.getInternalStructure().add(ret);
        
        ret.setName(sourcePort.getName());
        ret.setIsService(sourcePort.isIsService());
        ret.setIsBehavior(sourcePort.isIsBehavior());
        ret.setDirection(sourcePort.getDirection());
        
        createNotesFromNotes(session, sourcePort, ret);
        createStereotypesFromStereotypes(sourcePort, ret);
        createTaggedValuesFromTaggedValues(session, sourcePort, ret);
        return ret;
    }
