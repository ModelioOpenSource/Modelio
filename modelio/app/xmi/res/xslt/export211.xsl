...
<!--
<xsl:template match="//*[./eAnnotations/contents/@name = 'EcoreId']">  
	<xsl:copy>
		<xsl:attribute name="xmi:id" namespace="http://schema.omg.org/spec/XMI/2.1">
			<xsl:value-of select=" ./eAnnotations/contents[@name = 'EcoreId']/defaultValue/@value"/>
		</xsl:attribute>
    	<xsl:apply-templates select="@* |*| text()"/>
	</xsl:copy>
</xsl:template>
...
<xsl:template match="//eAnnotations[@source='Objing']/contents[@name = 'EcoreId']"/>

<xsl:template match="//eAnnotations[@source='Objing'][count(./contents) = 1][./contents[1]/@name = 'EcoreId']"/>	
-->
